        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('supervisor.task.update', $task->id) }}" method="POST">
                    ...
                    <input class="form-control" value="{{$task->name}}" id="name" name="name" type="text" placeholder="Task Name" required>
                    <label for="description" class="mt-4">Description</label>
                    <textarea class="form-control" id="description" name="description" placeholder="Description"
                              required>{{$task->description}}</textarea>
                    <label for="deadline" class="mt-4">Deadline</label>
                    <input value="{{$task->deadline}}" class="form-control" id="deadline" name="deadline" type="date" placeholder="Deadline" min="{{$currentperiod->start}}" max="{{$currentperiod->end}}"  required>
                    ...
                    <select name="pic" class="form-select w-100 mb-0">
                        @foreach ($task->projectuser->project->works as $student)
                            <option value="{{$student->id}}" @if ($task->projectuser->uci_user_id == $student->id) selected @endif>{{$student->name.' ('. $student->email .')'}}</option>
                        @endforeach
                    </select>
                    <div class="mt-3">
