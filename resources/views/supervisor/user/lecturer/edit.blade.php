...
@section('content')
    <div class="row">
        <div class="col-12 col-xl-8">
            <form action="{{ route('supervisor.user.update', $lecturer->id) }}" method="POST"
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input class="form-control" id="password" name="password" type="text" placeholder="Password"
                                   value="{{ $lecturer->password }}" required>
                        </div>
                    </div>
                    <div class="row">
                ...
                <div class="col-12 mb-0">
                    <div class="card border-light text-center p-0">
                        ...
                        <div class="card-body"><img
                            ...
                            <h4 class="h3">
                                {{ $lecturer->detailable->name }}
                            </h4>
                            <p class="text-gray mb-0">{{ $lecturer->detailable->nim }}</p>
                            <p class="text-gray mb-0">{{ $lecturer->detailable->department->name }}
                                ({{ $lecturer->detailable->department->initial }})
                            <p class="text-gray mb-0">{{ $lecturer->detailable->email }}</p>
                        </div>
                    </div>
                </div>
            </div>
